...
class Command {
  constructor(config) {
    ...
    this._readlineFactory =
      config.readlineFactory ||
      function() {
  ...
  init() {
    ...
    const readline = this._readlineFactory();

    const ask = () => {
      readline.question(
        'Will you be using ES modules (<script type="module">)? (y/n) ',
        answer => {
          if (answer === 'n' || answer === 'no') {
            writeConfig(defaultConfig());
            readline.close();
          } else if (answer === 'y' || answer === 'yes') {
            writeConfig(defaultEsmConfig());
            readline.close();
          } else {
            ask();
          }
