...
function Command(config) {
  ...
  this.run = async function(args) {
    ...
    if (!toRun) {
      ...
      return toRun.action.call(this);
    } else {
      return toRun.action.call(this, parseOptions(toRun, args.slice(1)));
    }
  };
}
...
async function serve(options) {
  await this.config.jasmineBrowser.startServer(this.loadConfig(options));
}

async function runSpecs(options) {
  await this.config.jasmineBrowser.runSpecs(this.loadConfig(options));
}
