...
class Runner {
  ...
  async run(runOptions) {
    ...
    if (runOptions.jsonDomReporter) {
      ...
      let batch1 = await getBatch(this._options.webdriver);
      proxyToReporter(batch1, this._options.reporter);
      if (isDone(batch1)) {
        return runDetails(batch1);
      } else {
        return await runTillEnd(
          this._options.webdriver,
          this._options.reporter
        );
      }
    }
  }
