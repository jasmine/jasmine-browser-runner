...
function runTillEnd(webdriver, reporter) {
  return new Promise(function(resolve, reject) {
    const intervalId = setInterval(async function() {
      try {
        // TODO: going async here means another of us can happen while we're waiting,
        // resulting in a getBatch call after we close
...
class Runner {
  ...
  async run(runOptions) {
    ...
    if (runOptions.jsonDomReporter) {
      ...
      proxyToReporter(results, this._options.reporter);
      ...
      return await runTillEnd(this._options.webdriver, this._options.reporter);
    }
  }
}
