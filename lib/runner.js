...
class Runner {
  ...
  async run(runOptions) {
    ...
    const loaded = await this._options.webdriver.executeScript('return !!document.querySelector(".jasmine_html-reporter")');

    if (!loaded) {
      console.error('Jasmine web page did not load');
      console.error('Expected to find jasmine_html-reporter in the page body but got:', await this._options.webdriver.executeScript('return document.body.innerHTML'));
