...
function runUntilEnd(webdriver, reporters) {
...
class Runner {
  ...
  async run(runOptions) {
    ...
    // Make sure the right page loaded. This provides better diagnostics if we
    // get something like a 500 error from Sauce Connect.
    const loaded = await this._options.webdriver.executeScript(
      'return !!document.querySelector("#jasmine_content")'
    );

    if (!loaded) {
      const body = await this._options.webdriver.executeScript(
        'return document.body.innerHTML'
      );
      console.error(
        'Jasmine web page did not load. Expected to find #jasmine_content in ' +
          'the page body but got:',
        body
      );
      throw new Error('Jasmine web page did not load');
    }

    return await runUntilEnd(this._options.webdriver, this._options.reporters);
  }
}
