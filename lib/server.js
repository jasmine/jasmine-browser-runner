...
class Server {
  ...
  importMap() {
    ...
    if (importMap.imports) {
      resultMap.imports = reifyRawSpecifierMap(importMap.imports, this.options);
    }
    ...
    if (importMap.scopes) {
      ...
      for (const [scope, map] of Object.entries(importMap.scopes)) {
        resultMap.scopes[scope] = reifyRawSpecifierMap(map, this.options);
      }
    }
...
function reifyRawSpecifierMap(rawSpecifierMap, options) {
  const concreteMap = {};

  for (const [key, value] of Object.entries(rawSpecifierMap)) {
    if (value.match(/^https?:\/\//)) {
      concreteMap[key] = value; // pass through unchanged
    } else if (fs.existsSync(path.join(options.srcDir, value))) {
      // Reuse existing __src__ path
      concreteMap[key] = './' + unWindows(path.join('__src__', value));
    } else if (fs.existsSync(path.join(options.specDir, value))) {
      // Reuse existing __spec__ path
      concreteMap[key] = './' + unWindows(path.join('__spec__', value));
    } else {
