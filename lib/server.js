...
class Server {
   ...
   * @returns importMapInfo | undefined
   */
  importMap() {
    ...
    if (importMap.scopes) {
      resultMap.scopes = {};
      for (let [scope, map] of Object.entries(importMap.scopes)) {
...
function reifyRawSpecifierMap(rawSpecifierMap) {
  const concreteMap = {};
  for (let [key, value] of Object.entries(rawSpecifierMap)) {
    if (value.match(/^https?:\/\//)) {
      concreteMap[key] = value; // pass through unchanged
    } else {
      concreteMap[key] = '.' + path.sep + path.join('__moduleRoot__', value);
    }
  }
  return concreteMap;
}
