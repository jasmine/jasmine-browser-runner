...
class Server {
  ...
  constructor(options) {
    ...
    this.hostname = findHostname(undefined, this.options.hostname);
  }
  ...
  start(serverOptions) {
    ...
    this.hostname = findHostname(serverOptions.hostname, this.options.hostname);
    return new Promise(resolve => {
      this._httpServer = app.listen(port, () => {
        const runningPort = this._httpServer.address().port;
        console.log(
          `Jasmine server is running here: http://${this.hostname}:${runningPort}`
        );
        console.log(
  ...
  /**
   * Gets the hostname that the server is requesting. The server must be started
   * before this method is called.
   * @function
   * @name Server#port
  ...
  hostname() {
    if (!this._httpServer) {
      throw new Error("Can't determine hostname before the server is started");
    }
...
function findHostname(serverHostname, optionsHostname) {
  if (typeof serverHostname !== 'undefined') {
    return serverHostname;
  }

  if (typeof optionsHostname !== 'undefined') {
    return optionsHostname;
  }
